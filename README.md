...
```
Команда создает статическое SPA-приложение: `index.html`, JS/CSS ассеты и `report.json` с полной моделью компонентов,
юнитов, внешних зависимостей, метрик и нарушений. Данные также встраиваются в `index.html`, поэтому отчет можно открыть
как обычный локальный HTML-файл без запуска сервера. Отчет работает локально без внешних CDN, показывает граф компонентов,
поиск по юнитам, фильтрацию по компонентам и отдельные списки нарушений/зависимостей.
